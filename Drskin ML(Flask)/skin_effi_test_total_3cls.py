...
args = easydict.EasyDict({
        'image_size': 224,
        'model_dir': './models/',
        # 'model_name': 'skincancer_classifier_refine.pt',
        'model_name': 'skincancer_classifier_refine_2.pt',
        # 'model_name': 'skincancer_classifier.pt',
        #
        # Valid Settings
        'batch_size': 1,
        'lr': 1e-4,
...
with torch.no_grad():
    for dirname, subdir, files in os.walk(test_dir):
        if '_compare' in dirname:
            continue
        if 'refine' in dirname:
            continue
        # ------------------------------------------------------------------
...
# pd_results.to_excel('../results/test_total_total.xlsx', index=False)
pd_results.to_excel('../results/test_total_refine_2.xlsx', index=False)
...
if False:
    if len(labels) == len(preds):
        ...
        # pd_conf_mat.to_excel('../results/conf_matrix_valid.xlsx', index=False)
        pd_conf_mat.to_excel('../results/conf_matrix_train.xlsx', index=False)
        #
    else:
        print(preds)

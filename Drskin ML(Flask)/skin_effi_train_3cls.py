...
args = easydict.EasyDict({
        ...
        'model_name': 'skincancer_classifier.pt',      # model 이름
        #
        # Training Settings
        'epochs': 1000,
        'batch_size': 10,
...
for dirname, subdirs, files in os.walk(args.data_dir + 'train/'):
    for img_file in files:
        if img_file.split('.')[-1].lower() not in ['jpg', 'png', 'gif']:        # image 파일이 아니면 무시
            continue
...
for dirname, subdirs, files in os.walk(args.data_dir + 'valid/'):
    for img_file in files:
        if img_file.split('.')[-1].lower() not in ['jpg', 'png', 'gif']:
            continue
...
# nohup python skin_effi_train.py   output.log 2&1 &
